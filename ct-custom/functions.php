...
function ct_custom_register_menus() {
  register_nav_menus(array(
    'primary' => __('Primary Menu', 'ct-custom'),
  ));
}
add_action('after_setup_theme', 'ct_custom_register_menus');
// Theme Settings Page
function ct_custom_theme_settings_page() {
  add_menu_page(
    ...
    'manage_options',
    'ct-theme-settings',
    'ct_custom_settings_page_html',
    'dashicons-admin-settings',
    99
  );
}
add_action('admin_menu', 'ct_custom_theme_settings_page');

function ct_custom_settings_page_html() { ?>
  <div class="wrap">
    <h1>Theme Settings</h1>
    <form method="post" action="options.php" enctype="multipart/form-data">
      <?php settings_fields('ct_theme_settings_group'); ?>
      <?php do_settings_sections('ct_theme_settings_group'); ?>
      <table class="form-table">
        <tr>
          <th>Logo</th>
          <td><input type="text" name="ct_logo" value="<?php echo get_option('ct_logo'); ?>" placeholder="Logo URL"/></td>
        </tr>
        <tr>
          <th>Phone Number</th>
          <td><input type="text" name="ct_phone" value="<?php echo get_option('ct_phone'); ?>"/></td>
        </tr>
        <tr>
          <th>Address</th>
          <td><textarea name="ct_address"><?php echo get_option('ct_address'); ?></textarea></td>
        </tr>
        <tr>
          <th>Fax Number</th>
          <td><input type="text" name="ct_fax" value="<?php echo get_option('ct_fax'); ?>"/></td>
        </tr>
        <tr>
          <th>Facebook URL</th>
          <td><input type="text" name="ct_facebook" value="<?php echo get_option('ct_facebook'); ?>"/></td>
        </tr>
        <tr>
          <th>Twitter URL</th>
          <td><input type="text" name="ct_twitter" value="<?php echo get_option('ct_twitter'); ?>"/></td>
        </tr>
      </table>
      <?php submit_button(); ?>
    </form>
...
function ct_custom_register_settings() {
  ...
  register_setting('ct_theme_settings_group', 'ct_facebook');
  register_setting('ct_theme_settings_group', 'ct_twitter');
}
